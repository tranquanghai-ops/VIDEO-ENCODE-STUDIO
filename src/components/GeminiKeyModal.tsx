...
export const GeminiKeyModal: React.FC<GeminiKeyModalProps> = ({ isOpen, onClose, onConnected }) => {
  ...
  useEffect(() => {
    if (!isOpen) return;
    refresh(); setName(''); setInputKey(''); setModalError(''); setShowPassword(false); setEditingId('');
  }, [isOpen]);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const profile = editingId ? updateStoredApiKey(editingId, clean, name, rememberOption) : saveStoredApiKey(clean, rememberOption, name);
      if (!profile) { setModalError('Không thể lưu API Key trên trình duyệt này.'); return; }
      const next = refresh();
      onConnected(profile, next.length);
      setName(''); setInputKey(''); setEditingId('');
    } catch (error) {
  ...
  const handleEdit = (profile: GeminiKeyProfile) => {
    setEditingId(profile.id); setName(profile.name); setInputKey(profile.key);
    setRememberOption(profile.type === 'local'); setModalError('');
  };
  ...
  return <div className="key-modal-backdrop" role="dialog" aria-modal="true" aria-label="Quản lý Gemini API Key">
    <section className="key-modal">
      ...
      <footer>{editingId && <button type="button" className="secondary" onClick={() => { setEditingId(''); setName(''); setInputKey(''); }}>Hủy sửa</button>}<button type="button" className="secondary" onClick={onClose} disabled={isValidating}>Đóng</button><button type="button" className="primary" onClick={handleConnect} disabled={isValidating}>{isValidating ? 'Đang kiểm tra…' : editingId ? 'Kiểm tra & Lưu' : 'Kiểm tra & Thêm key'}</button></footer>
    </section>
  </div>;
};
